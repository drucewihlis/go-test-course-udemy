...
func Test_app_refreshUsingCookie(t *testing.T) {
	...
	for _, e := range tests{
		...
		if rr.Code != e.expectedStatus {
			t.Errorf("%s: wrong stat code returned; expected %d but got %d", e.name, e.expectedStatus, rr.Code)
		}
	}
}
	...
	for _, c := range rr.Result().Cookies() {
		if c.Name == "__Host-refresh_token" {
			...
			if c.Expires.After(time.Now()) {
				t.Errorf("cookie expiration in future, but shouuld not be: %v", c.Expires.UTC()) 
			}
		}
	}

	if !foundCookie {
		t.Error("__Host-refresh_token cookie not found in response")
	}
}